...
function randomColorList(count, clearer) {
  ...
  list.sort(() => Math.random() > 0.5).forEach(c => {
...
export default class Chart extends alaska.Model {
  ...
  async getData(ctx) {
    ...
    if (chart.populated('sources')) {
      ...
      for (let id of chart.sources) {
        sources.push(await ChartSource.findById(id));
      }
    }
    ...
    for (let source of sources) {
      ...
      if (chart.type === 'line' && source.type === 'time' && data.length) {
        ...
        data = Object.keys(dataMap).sort().map(x => {
          let y = dataMap[x];
          x = new Date(x * 1);
          return { x, y };
        });
